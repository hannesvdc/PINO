...
def precompute_covariance(x_grid : pt.Tensor, 
    ...
    grid_1, grid_2 = pt.meshgrid(x_grid, x_grid)
...
def gp(x_grid : pt.Tensor,
    ...
    K_cond = K_ii - K_ib @ K_bb_inv @ K_bi + 1e-8 * pt.eye(N-2) 

    # Sample the conditional GP
    conditional_normal = pt.distributions.MultivariateNormal( pt.zeros((N-2,)), K_cond )
    u0 = conditional_normal.rsample( (N-2,) )
    return pt.stack(( pt.tensor([0.0]), u0, pt.tensor([0.0]) ), dim=0)