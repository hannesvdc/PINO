...
ax1.pcolormesh( X_mlp.numpy(), Y_mlp.numpy(), T_sol_mlp.T.numpy(), vmin=v_min, vmax=v_max , cmap='jet')
ax1.set_title("MLP Embedding")
ax1.set_xlabel(r"$x$")
ax1.set_ylabel(r"$x$")
X_conv, Y_conv = pt.meshgrid( x_grid, tau_fd_conv, indexing="ij" )
ax2.pcolormesh( X_conv.numpy(), Y_conv.numpy(), T_sol_conv.T.numpy(), vmin=v_min, vmax=v_max , cmap='jet')
ax2.set_title("Convolution Embedding")
ax2.set_xlabel(r"$x$")
X_film, Y_film = pt.meshgrid( x_grid, tau_fd_film, indexing="ij" )
ax3.pcolormesh( X_film.numpy(), Y_film.numpy(), T_sol_film.T.numpy(), vmin=v_min, vmax=v_max , cmap='jet')
ax3.set_title("FiLM")
ax3.set_xlabel(r"$x$")
...
ax1.pcolormesh( X_mlp.numpy(), Y_mlp.numpy(), mlp_rel_err.T.numpy(), vmin=v_min, vmax=v_max , cmap='jet')
ax1.set_title("MLP Embedding")
ax1.set_xlabel(r"$x$")
ax1.set_ylabel(r"$x$")
ax2.pcolormesh( X_conv.numpy(), Y_conv.numpy(), conv_rel_err.T.numpy(), vmin=v_min, vmax=v_max , cmap='jet')
ax2.set_title("Convolution Embedding")
ax2.set_xlabel(r"$x$")
m = ax3.pcolormesh( X_film.numpy(), Y_film.numpy(), film_rel_err.T.numpy(), vmin=v_min, vmax=v_max , cmap='jet')
ax3.set_title("FiLM")
ax3.set_xlabel(r"$x$")
plt.colorbar( m )
plt.show()