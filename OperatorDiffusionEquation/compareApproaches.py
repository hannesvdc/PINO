...
ax1.pcolormesh( X_mlp.cpu().numpy(), Y_mlp.cpu().numpy(), T_sol_mlp.T.cpu().numpy(), vmin=v_min, vmax=v_max , cmap='jet', shading="nearest")
ax1.set_title("MLP Embedding")
ax1.set_xlabel(r"$x$")
ax1.set_ylabel(r"$x$")
X_conv, Y_conv = pt.meshgrid( x_grid, tau_fd_conv, indexing="ij" )
ax2.pcolormesh( X_conv.cpu().numpy(), Y_conv.cpu().numpy(), T_sol_conv.T.cpu().numpy(), vmin=v_min, vmax=v_max , cmap='jet', shading="nearest")
ax2.set_title("Convolution Embedding")
ax2.set_xlabel(r"$x$")
X_film, Y_film = pt.meshgrid( x_grid, tau_fd_film, indexing="ij" )
ax3.pcolormesh( X_film.cpu().numpy(), Y_film.cpu().numpy(), T_sol_film.T.cpu().numpy(), vmin=v_min, vmax=v_max , cmap='jet', shading="nearest")
ax3.set_title("FiLM")
ax3.set_xlabel(r"$x$")
...
ax1.pcolormesh( X_mlp.cpu().numpy(), Y_mlp.cpu().numpy(), mlp_rel_err.T.cpu().numpy(), vmin=v_min, vmax=v_max , cmap='jet', shading="nearest")
ax1.set_title("MLP Embedding")
ax1.set_xlabel(r"$x$")
ax1.set_ylabel(r"$x$")
ax2.pcolormesh( X_conv.cpu().numpy(), Y_conv.cpu().numpy(), conv_rel_err.T.cpu().numpy(), vmin=v_min, vmax=v_max , cmap='jet', shading="nearest")
ax2.set_title("Convolution Embedding")
ax2.set_xlabel(r"$x$")
m = ax3.pcolormesh( X_film.cpu().numpy(), Y_film.cpu().numpy(), film_rel_err.T.cpu().numpy(), vmin=v_min, vmax=v_max , cmap='jet', shading="nearest")
ax3.set_title("FiLM")
ax3.set_xlabel(r"$x$")
plt.colorbar( m )
plt.show()