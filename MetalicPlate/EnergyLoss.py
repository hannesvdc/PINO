...
class EnergyLoss( nn.Module ):
    ...
    def forward(self, model : nn.Module,
        ...
        # Safety
        x = x.requires_grad_(training)
        y = y.requires_grad_(training)
        nu = nu.requires_grad_(False)
        gx = gx.requires_grad_(False)
        gy = gy.requires_grad_(False)

        # Choose gradient function based on mode
        grad_fn = grads_uv_autograd if training else grads_uv_no_grad

        # Accumulators for logging (detached, no graph)
        total_energy = 0.0
        total_boundary = 0.0
        total_traction = 0.0
        total_weight = 0.0
        ...
        # Each chunk's loss is backpropagated immediately to free the graph and save memory.
        for s in range(0, Bb, self.branch_chunk):
            ...
            _, _, u_x, u_y, v_x, v_y = grad_fn(model, x, y, nu_c, gx_c, gy_c)  # (Bc,Bt)

            # Boundary values only (no spatial derivatives needed)
            bc_x = pt.ones_like( bc_y )
            u_bc, v_bc, u_xb, u_yb, v_xb, v_yb = grad_fn(model, bc_x, bc_y, nu_c, gx_c, gy_c)
            ...
            # Compute this chunk's loss and backprop immediately to free the graph
            chunk_energy = w * strain_integral.mean()
            chunk_boundary = w * bc_integral.mean()
            chunk_traction = w * traction_mse.mean()
            chunk_loss = chunk_energy - chunk_boundary + self.lambda_trac * chunk_traction
            if training:
                chunk_loss.backward()

            # Log detached values
            total_energy += float(chunk_energy.detach().item())
            total_boundary += float(chunk_boundary.detach().item())
            total_traction += float(chunk_traction.detach().item())
            total_weight += w

            # free refs early for memory
            del u_x, u_y, v_x, v_y, u_bc, v_bc, gx_int, gy_int, strain_energy
            del chunk_loss, chunk_energy, chunk_boundary, chunk_traction

        avg_energy = total_energy / total_weight
        avg_boundary = total_boundary / total_weight
        avg_traction = total_traction / total_weight
        loss_val = avg_energy - avg_boundary + self.lambda_trac * avg_traction
        loss_info = {"energy" : avg_energy, "boundary" : avg_boundary,
                     "rms" : loss_val, "traction" : avg_traction}
        return loss_val, loss_info
...
def grads_uv_no_grad(model, x, y, nu, gx, gy):
    """
    Same as grads_uv_autograd but without create_graph (for validation).
    Uses less memory since no second-order graph is built.
    """
    if x.ndim == 1: x = x[:, None]
    if y.ndim == 1: y = y[:, None]
    if nu.ndim == 1: nu = nu[:, None]
    ...
    for b in range(Bb):
        u_x_b, u_y_b = pt.autograd.grad(u[b].sum(), [x, y], retain_graph=True)
        v_x_b, v_y_b = pt.autograd.grad(v[b].sum(), [x, y], retain_graph=True)
        u_x_all.append(u_x_b[:, 0])
        u_y_all.append(u_y_b[:, 0])
        v_x_all.append(v_x_b[:, 0])
        v_y_all.append(v_y_b[:, 0])

    return u.detach(), v.detach(), pt.stack(u_x_all).detach(), pt.stack(u_y_all).detach(), pt.stack(v_x_all).detach(), pt.stack(v_y_all).detach()